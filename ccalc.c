...
cmd2func(char c) {
  ...
  case 'S':
    // this smells fishy ;-)
    func = (calc_simple_func_ptr)&csum;
    break;
  case 'U':
...
writeResults(long long result) {
  ...
  // force crash -- replace with your code here
  //first case that SUM_POSITIVE failed
  if (write(1, &SUM_POSITIVE, sizeof(SUM_POSITIVE)) != sizeof(SUM_POSITIVE)) {
    fprintf(stderr, "ERROR: write SUM_POSITIVE failed\n");
  }
  //second case that SUM_NEGATIVE failed
  if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) {
    fprintf(stderr, "ERROR: write SUM_NEGATIVE failed\n");
  }
  //third case that data_between failed
  long long data_between = ((void*)&CALC_DATA_END)-((void*)&CALC_DATA_BEGIN);
  if (write(1, (void*)&CALC_DATA_BEGIN, data_between) != data_between) {
    fprintf(stderr, "ERROR: write data_between failed\n");
  }
}

...
cpopcnt(long long result, long long y)
{
  ...
  for(int i = 0; i < 8*sizeof(v); i++){//Loop: Runs for each bit in the variable 'v'. The size of 'v' is multiplied by 8 (as there are 8 bits in a byte) to get the total number of bits.
    cnt = cnt + (v & 0x1);// Bitwise AND operation with 0x1 (which is binary 0001) to check the least significant bit of 'v'. The result (0 or 1) is added to the 'cnt' variable.
    v=v>>1;//Right shift operation on 'v'.
  }
  return result + cnt;
}
